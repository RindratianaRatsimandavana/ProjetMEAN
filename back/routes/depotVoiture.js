...
router.patch('/:depVoitId/:cible/:valUpdate',function(req,res,next)
    {
        ...
        if(req.params.cible === "etatReception") //receptionner voiture 
        {
            ...
            Depotvoiture.update({_id: depVoitId},{$set: {etatReception:Number(req.params.valUpdate)}}).exec().then(result => {
                console.log(result);
                res.status(200).json(result);
        ...
        if(req.params.cible === "etatRecuperation") //etatRecuperation voiture 
        {
            ...
            //http://localhost:3000/depotVoiture/63c662a01d3459b5410a0d44/etatReception/1
                                                                        //ovaina 1 ny etatreception satria recu ny voiture
            Depotvoiture.update({_id: depVoitId},{$set: {etatRecuperation:Number(req.params.valUpdate)}}).exec().then(result => {
                console.log(result);
                res.status(200).json(result);
                //Eto tokony misy fandefasana mail ny client
            }).catch(err => {
        ...
        else if(req.params.cible === "bonDeSortie") //receptionner voiture 
        {
            ...
            //http://localhost:3000/depotVoiture/63c662a01d3459b5410a0d44/etatReception/1
                                                                        //ovaina 1 ny etatreception satria recu ny voiture
            Depotvoiture.update({_id: depVoitId},{$set: {bonDeSortie:Number(req.params.valUpdate)}}).exec().then(result => {
                console.log(result);
                res.status(200).json(result);
                //Eto tokony misy fandefasana mail ny client
            }).catch(err => {
...
router.get('/date',function(req,res,next)
    {
        ...
        Depotvoiture.find({etatReception:1}) 
            .exec().then(result => {
                console.log('avant');
                let sommeDiffDate= 0;
                ...
                for(let i=0;i<result.length;i++)
                {
                    let diff = result[i].dateDepotVoit.getTime() - result[i].dateDepotVoit.getTime();   
            ...
            res.status(200).json(
                {
                    ...
                    moyennetMoy: moyenneMaj,
                    moyenne: moyenneMaj,
                    moyenneMin: moyenneMaj
                }
            );
        }).catch(err => {
            console.log(err);
